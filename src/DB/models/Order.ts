...
export class Order {
    ...
    @OneToMany(() => OrderItem, order => order.Order, { cascade: ['insert','update'] })
    orderItems?: OrderItem[];

    @Column()
    customerId?: number;
    @ManyToOne(() => Customer, customer => customer.Orders, { cascade: ['insert'],onDelete:'CASCADE' })
    customer?: Customer;
    ...
    @Column({ nullable: true })
    getirOrderId?: number;
    @OneToOne(() => GetirOrderDetails, getirOrder => getirOrder.Order, { cascade: ['insert','update'], nullable: true,onDelete:'CASCADE' })
    @JoinColumn({name:'getirOrderId', referencedColumnName: 'id',})
    GetirOrder?: GetirOrderDetails;
    @OneToOne(() => TelegramOrder, telegramOrder => telegramOrder.Order, { cascade: ['insert','update'], nullable: true,onDelete:'CASCADE' })
