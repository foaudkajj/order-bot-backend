...
export default class GetirToken {
    ...
    private static async getAndUpdateToken(merchant: Merchant) {
        ...
        const response = await httpService.post(process.env.GetirApi + Endpoints.auth, { appSecretKey: merchant.GetirAppSecretKey, restaurantSecretKey: merchant.GetirRestaurantSecretKey }).toPromise();
        if (response.status == 200) {
            const merchantRepository = getCustomRepository(MerchantRepository);
            merchantRepository.update({ Id: merchant.Id }, { GetirAccessToken: response.data.token, GetirTokenLastCreated: new Date() })
