...
export class OrderService {
  ...
  async Get(query: DataSourceLoadOptionsBase) {
    let entities: Order[];
    let findOptions: FindManyOptions<Order> = this.devextremeLoadOptions.GetFindOptionsFromQuery(
      query,
    );
    ...
    let response: UIResponseBase<Order> = {
      IsError: false,
      data: entities,
  ...
  async Update(updateDetails: Order) {
    try {
      let entity = await getRepository(Order).findOne({
        where: {Id: updateDetails.Id},
      });
      let {Id, customer, ...updatedEntity} = {...entity, ...updateDetails};

      let userEntity: Order = await getRepository(Order).findOne(
        {Id: entity.Id},
        {relations: ['customer']},
      );
      if (customer) {
