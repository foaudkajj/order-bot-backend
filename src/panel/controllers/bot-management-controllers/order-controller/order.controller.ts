...
export class OrderController {
    ...
    async Insert(@Body() request): Promise<UIResponseBase<Order>> {
        let entity = JSON.parse(request.values) as Order;
        let result = await this.orderService.Insert(entity);
        return result;
    }

    @Post('Update')
    @PermessionsGuard(PermessionEnum.UPDATE_ORDER)
    async Update(@Body() request: DxGridUpdateRequest): Promise<UIResponseBase<Order>> {
        let entity = { ...JSON.parse(request.values) } as Order;
        entity.Id = request.key;
        let result = await this.orderService.Update(entity);
        return result;
    }

    @Post('Delete')
    @PermessionsGuard(PermessionEnum.DELETE_ORDER)
    async Delete(@Body() deleteRequest: DxGridDeleteRequest): Promise<UIResponseBase<Order>> {
        let result = await this.orderService.Delete(deleteRequest.key);
        return result;
    }
}