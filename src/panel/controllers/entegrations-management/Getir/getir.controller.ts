import { Body, Controller, Get, Delete, Post, Request, Headers } from '@nestjs/common';
...
export class GetirController {
    ...
    async ActivateInActiveOptions(@Request() request, @Body() body): Promise<UIResponseBase<any>> {
        const { MerchantId } = request.user;
        let result = await this.getirService.ActivateInActiveOptionProducts(MerchantId, body);
        return result;
    }
    ...
    async UpdateOptionStatusInSpecificProductAndCategory(@Request() request, @Body() body): Promise<UIResponseBase<any>> {
        const { MerchantId } = request.user;
        let result = await this.getirService.UpdateOptionStatusInSpecificProductAndCategory(MerchantId, body);
        return result;
    }
    ...
    @Post('OrderReceived')
    @AllowAnonymous()
    async OrderReceived(@Body() body: FoodOrderDto, @Request() request, @Headers() Headers): Promise<string> {
        // const { MerchantId } = request.user;
        console.log(Headers)
    ...
    @Post('OrderCanceled')
    @AllowAnonymous()
    async OrderCanceled(@Request() request, @Headers() Headers, @Body() body): Promise<string> {
        // const { MerchantId } = request.user;
        console.log(Headers)
