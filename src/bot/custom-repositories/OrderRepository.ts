...
export class OrderRepository extends Repository<Order> {
    async getOrdersInBasketByStatus(ctx: BotContext, orderStatus: OrderStatus, relations?: string[]) {
        ...
        const order = (await this.findOne({ where: { customerId: customer.Id, OrderStatus: orderStatus }, relations: relations, order: { CreateDate: 'DESC' } }));
        return order;
    }
    ...
    async getOrderInBasketByTelegramId(ctx: BotContext, relations?: string[]) {
        ...
        if (relations && relations.length > 0) {
            return await this.findOne({ where: { customerId: customer.Id,OrderStatus: OrderStatus.New },relations: relations });
        } else {
            return await this.findOne({ where: { customerId: customer.Id,OrderStatus: OrderStatus.New } });
        }
    }
